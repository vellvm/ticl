(** * Layout: a generic queue constructor and the concrete rotating queue.

    SHARED between the two proof families.  Everything here is a construction
    or an elementary fact about it; no separation-logic notion appears.

    [qheap hdr ns vs] is the heap that IS the queue [ns]/[vs] headed at [hdr].
    Proving [qrep] and [qex] for it once means every per-variant file contains
    an address layout and a payload list, and no heap reasoning at all. *)
...
(** ** Two elementary frame shapes *)

Definition hsingle (a v: nat) : Heap :=
  fun x => if Nat.eqb x a then Some v else None.

Lemma hsingle_dom: forall a v x, hsingle a v x <> None -> x = a.
Proof.
  intros a v x H; unfold hsingle in H.
  destruct (Nat.eqb_spec x a) as [Ex | Hne]; [exact Ex | congruence].
Qed.
...
(** The uniform vocabulary in which a frame's CONTENT is described: a list of
    (address, value) pairs the frame holds.  Both proof families state "the
    frame is intact" as [cellsat Wcells h]. *)
...
(** ** PCM facts about the disjoint-heap monoid.

    [hdisj]/[hunion] come from the frozen [HeapQ.v] resource model; these are
    the four elementary consequences both proof families use. *)
