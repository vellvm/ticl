...
Section ObservedScheduler.
  ...
  Lemma forget_scheduler_offers_focused_ret_projection n
    ...
    forget_scheduler_offers (schedule_with_offers n ((v -- i)) None).
  Proof.
    intro Hret.
    unfold forget_scheduler_offers.
    rewrite unfold_interp.
  ...
  Lemma forget_scheduler_offers_focused_guard_projection n
    ...
    forget_scheduler_offers (schedule_with_offers (S n) v (Some i)) ~
    forget_scheduler_offers
      (schedule_with_offers (S n) ((v @ i := t)) (Some i)).
  Proof.
    intro Hguard.
    unfold forget_scheduler_offers.
    rewrite unfold_interp.
  ...
  Lemma forget_scheduler_offers_focused_yield_projection n
    ...
    forget_scheduler_offers (schedule_with_offers (S n) v (Some i)) ~
    forget_scheduler_offers
      (schedule_with_offers (S n) ((v @ i := (k tt))) None).
  Proof.
    intro Hyield.
    unfold forget_scheduler_offers.
    rewrite unfold_interp.
  ...
  Theorem return_removes_live_slot n
      (v : pool E (S n)) (i : Fin.t (S n)) :
    observe (v $ i) = RetF tt ->
    ReturnSlotTransport v i (v -- i).
  Proof.
    intro Hret; clear Hret.
    revert v i.
    induction n as [| n IH]; intros v i.
    - constructor.
      + intro j_new. inversion j_new.
      + intros j_old Hneq.
        contradiction Hneq. apply live_slot_one_eq.
    - refine (Vector.caseS' v
        (fun v0 => ReturnSlotTransport v0 i (v0 -- i)) _).
      clear v; intros h tl.
      refine (Fin.caseS' i
        (fun i0 => ReturnSlotTransport ((h :: tl)%vector) i0
                     (((h :: tl)%vector) -- i0)) _ _); clear i.
      + rewrite vector_remove_head.
        constructor.
        * intro j_new.
          exists (Fin.FS j_new). split; [discriminate | reflexivity].
        * intros j_old Hneq.
          refine (Fin.caseS' j_old
            (fun j => j <> Fin.F1 ->
               exists j_new : LiveSlot (S n),
                 (tl $ j_new) = (((h :: tl)%vector) $ j)) _ _ Hneq).
          -- intro Hcontra. contradiction Hcontra; reflexivity.
          -- intros j_new _. exists j_new. reflexivity.
      + intro i_tail.
        rewrite vector_remove_tail.
        destruct (IH tl i_tail) as [IHnew IHold].
        constructor.
        * intro j_new.
          refine (Fin.caseS' j_new
            (fun j => exists j_old : LiveSlot (S (S n)),
               j_old <> Fin.FS i_tail /\
               (((h :: (tl -- i_tail))%vector) $ j)
                 = (((h :: tl)%vector) $ j_old)) _ _).
          -- exists Fin.F1. split; [discriminate | reflexivity].
          -- intro j_new'.
        ...
        * intros j_old Hneq.
          refine (Fin.caseS' j_old
            (fun j => j <> Fin.FS i_tail ->
               exists j_new : LiveSlot (S n),
                 (((h :: (tl -- i_tail))%vector) $ j_new)
                   = (((h :: tl)%vector) $ j)) _ _ Hneq).
  ...
  Proof.
    intro Hneq.
    rewrite shift_live_slot_across_spawn_tail.
    apply Vector.nth_replace_neq. congruence.
  Qed.
  ...
  Theorem transport_live_slots_across_spawn n
      (v : pool E (S n)) (i : LiveSlot (S n)) k :
    observe (v $ i) = VisF (inr (inl Fork)) k ->
    SpawnSlotTransport v i k ((k true :: (v @ i := (k false)))%vector).
  Proof.
    intro Hfork; clear Hfork.
    constructor.
    - reflexivity.
    - exact (Vector.nth_replace_eq _ _ i v (k false)).
    - intros j Hneq.
      exact (Vector.nth_replace_neq _ _ j i Hneq v (k false)).
  Qed.
