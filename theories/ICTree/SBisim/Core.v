...
Notation "t ~ u" := (sbisim eq t u) (at level 70): ictree_scope.
Notation "t (~ L ) u" := (sbisim L t u) (at level 70): ictree_scope.
Notation "t [~ L] u" := (st L _ t u) (at level 78): ictree_scope.
Notation "t [~] u" := (st eq _ t u) (at level 78): ictree_scope.
Notation "t {~ L } u" := (sbt L _ t u) (at level 79): ictree_scope.
Notation "t {~} u" := (sbt eq _ t u) (at level 79): ictree_scope.
Notation "t {{ ~ L }} u" := (sb L _ t u) (at level 79): ictree_scope.
Notation "t {{~}} u" := (sb eq _ t u) (at level 79): ictree_scope.
...
|*)
  ...
  Program Definition sbisim_clos2 {LE LF} : mon (rel (ictree E X) (ictree F Y)) :=
...
Proof.
  split; intros; subst; auto. 
Defined.
Global Hint Resolve respects_val_eq respects_tau_eq: ictree.
...
Section bind.
  ...
  Program Definition bind_ctx_sbisim L0 : mon (rel (ictree E X') (ictree F Y')) :=
    {|body := fun R => @bind_ctx2 E F HE HF X Y X' Y' (sbisim L0)
                              (pointwise R0 R) |}.
  Next Obligation.
    intros ??? H. apply leq_bind_ctx2. intros ?? H' ?? H''.
...
Lemma st_clo_bind_gen {E F: Type} {HE: Encode E} {HF: Encode F} {X Y X' Y': Type}
  ...
  (k1 : X -> ictree E X') (k2 : Y -> ictree F Y') RR:
  t1 (~L0) t2 ->
...
Lemma st_clo_bind_eq {E: Type} {HE: Encode E} {X X': Type}
      (t1 : ictree E X) (t2: ictree E X)
      (k1 : X -> ictree E X') (k2 : X -> ictree E X') RR:
  t1 ~ t2 ->
...
Lemma sbisim_clo_bind_gen `{HE: Encode E} `{HF: Encode F} {X Y X' Y': Type} {L : rel (label E) (label F)}
      ...
      (t1 : ictree E X) (t2: ictree F Y)
      (HL0 : is_update_val_rel L R0 L0)
      (k1 : X -> ictree E X') (k2 : Y -> ictree F Y') :
  t1 (~L0) t2 ->
...
Lemma sbisim_clo_bind_eq `{HE: Encode E} {X X': Type}
      (t1 : ictree E X) (t2: ictree E X)
      (k1 : X -> ictree E X') (k2 : X -> ictree E X') :
  t1 ~ t2 ->
  (forall x, k1 x ~ k2 x) ->
  t1 >>= k1 ~ t2 >>= k2.
...
Ltac __upto_bind_sbisim :=
  match goal with
    |- @sbisim ?E ?F ?C ?D ?X ?Y ?L
              (ICtree.bind (X := ?T) _ _) (ICtree.bind (X := ?T) _ _) => apply sbisim_clo_bind_eq
  | |- body (t (@sb ?E ?F ?C ?D ?X ?Y ?L)) ?R
           (ICtree.bind (X := ?X') _ _) (ICtree.bind (X := ?X') _ _) =>
...
Ltac __upto_bind_eq_sbisim :=
  match goal with
  | |- @sbisim ?E ?F ?C ?D ?X ?Y eq (ICtree.bind (X := ?Z) _ _) (ICtree.bind (X := ?Z) _ _) =>
...
Section Ctx.
  ...
  Definition vis_ctx (e : E) (f: F)
             (R: rel (encode e -> ictree E X') (encode f -> ictree F Y')):
    rel (ictree E X') (ictree F Y') :=
    sup_all (fun k => sup (R k) (fun k' => pairH (Vis e k) (Vis f k'))).
  
  Lemma leq_vis_ctx (e: E) (f: F):
    forall (R: rel (encode e -> ictree E X') (encode f -> ictree F Y')) R', vis_ctx R <= R' <->
...
Section Proof_Rules.
  ...
  Lemma step_sb_ret_gen(x: X) (y: Y) (R : rel (ictree E X) (ictree F Y)) :
    R stuck stuck ->
    L (val x) (val y) ->
  ...
  Lemma step_sb_ret (x: X) (y: Y) (R : rel (ictree E X) (ictree F Y)) :
    L (val x) (val y) ->
    sbt L R (Ret x) (Ret y).
  Proof.
    intros LH; subst.
  ...
  Lemma step_sb_vis {X' Y'} (e: E) (f: F) (k: encode e -> ictree E X') (k': encode f -> ictree F Y') {R : rel _ _}:
  ...
  Lemma step_sb_step (t : ictree E X) (t' : ictree F Y) (R: rel _ _) :
    L tau tau ->
    (st L R t t') ->
  ...
  Lemma step_sb_br_gen {n m}
    (k : fin' n -> ictree E X) (k': fin' m  -> ictree F Y) (R : rel _ _):
    (Proper (equ eq ==> equ eq ==> impl) R) ->
    L tau tau ->
  ...
  Lemma step_sb_br_id_gen {n} 
        (k : fin' n -> ictree E X) (k': fin' n -> ictree F Y) (R: rel _ _) :
    (Proper (equ eq ==> equ eq ==> impl) R) ->
    L tau tau ->
    (forall x, R (k x) (k' x)) ->
  ...
  Lemma step_sb_guard (t : ictree E X) (t' : ictree F Y) (R: rel _ _) :
    sbt L R t t' ->
...
Section Sb_Proof_System.
  ...
  Lemma sb_ret: forall x y,
      x = y ->
      (Ret x: ictree E X) ~ Ret y.
  Proof.
    intros * EQ; step.
    now apply step_sb_ret; subst.
  Qed.

  Lemma sb_vis {Y}: forall (e: E) (k k': encode e -> ictree E Y),
      (forall x, k x ~ k' x) ->
      Vis e k ~ Vis e k'.
  Proof.
    intros.
 ...
 Lemma sb_guard_l: forall (t u : ictree E X),
      t ~ u ->
      Guard t ~ u.
  Proof.
  ...
  Lemma sb_guard_r: forall (t u : ictree E X),
      t ~ u ->
      t ~ Guard u.
  Proof.
    intros * EQ; now rewrite sb_guard.
  ...
  Lemma sb_guard_lr: forall (t u : ictree E X),
      t ~ u ->
      Guard t ~ Guard u.
  Proof.
    intros * EQ; now rewrite !sb_guard.
  ...
  Lemma sb_step: forall (t u : ictree E X),
      t ~ u ->
      step t ~ step u.
  Proof.
    intros * EQ; step.
  ...
  Lemma sb_br {n m} (k : fin' n -> ictree E X) (k': fin' m -> ictree E X) :
  ...
  Lemma sb_br_id {n} (k k' : fin' n -> ictree E X) :
    (forall x, k x ~ k' x) ->
    Br n k ~ Br n k'.
  Proof.
    intros * EQs; step.
  ...
  Lemma sb_unfold_forever{Y}: forall (k: Y -> ictree E Y)(i: Y),
...
Section SanityChecks.
  ...
  Lemma sbisim_ret_inv (r1 r2 : X) :
    (Ret r1 : ictree E X) ~ Ret r2 -> r1 = r2.
  Proof.
    intro.
    eplayL.    
    trans in TR.
  ...
  Lemma sbisim_vis_invE (e: E) (k1 k2: encode e -> ictree E X) (x: encode e):
    Vis e k1 ~ Vis e k2 ->
    forall x, k1 x ~ k2 x.
  Proof.
    intros.
  ...
  Lemma sbisim_br_inv {Y n m} (k1 : fin' n -> ictree E X) (k2: fin' m -> ictree E Y) :
    Br n k1 ~ Br m k2 ->
  ...
  Lemma sbisim_ret_vis_inv (r : X) (e : E) (k : encode e -> ictree E X) :
    Ret r ~ Vis e k -> False.
  Proof.
    intros * abs.
    eplayL.
  ...
  Lemma sbisim_ret_br_inv {n} (r: X) (k : fin' n -> ictree E X) :
    Ret r ~ Br n k -> False.
  Proof.
    intros * abs.
    eplayL.
  ...
  Lemma sbisim_vis_br_inv{n} (e: E) (k1 : encode e -> ictree E X) (k2: fin' n -> ictree E X):
