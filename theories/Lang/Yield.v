...
Inductive stmt : Type :=
| Seq    (a b : stmt)            (* a ; b *)
| Assign (x: var) (e: expr)      (* a := x *)
| If     (i : cond) (t e : stmt) (* if (i) then { t } else { e } *)
| While  (t : cond) (b : stmt)   (* while (t) { b } *)
...
Variant memE : Type :=
  | wr (id: var) (v: value): memE
  | rd (id: var): memE.

Global Instance encode_memE: Encode memE :=
  fun e => match e with
        | wr _ _ => unit
        | rd _ => nat
        end.
...
Section Denote1.
  ...
  Fixpoint denote_expr (e : expr) : ctree E value :=
    ...
    | Minus a b => l <- denote_expr a ;; r <- denote_expr b ;; Ret (l - r)%nat
  ...
  Fixpoint denote_imp (s : stmt) : thread :=
    match s with
    | Assign x e =>  v <- denote_expr e ;; u <- write x v ;; Ret u
